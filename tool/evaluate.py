...
def get_args_parser():
    ...
    parser.add_argument('--data_path', default='F:/ShanghaiTech/testing/frames', type=str,
                        help='Please specify path to the ImageNet training data.')
    parser.add_argument('--label_path', default='F:\ShanghaiTech/testing/test_frame_mask', type=str)
    parser.add_argument('--model_pretrain', default='F:\My-repository\My_Model\pretrain_model\I3D_Swin_Cluster_SpaceCluster_TimeDownSample.pth',
...
def predict(model, recon_loss, data_loader, dataset, data_iter):
    ...
    for it, (images, idx, label, scene_num) in enumerate(data_loader):
        ...
        while index + args.frame_num < len(images[0, 0, :]):
            ...
            label_clip = label[:, index: index + args.frame_num]
            index = index + args.frame_num
            clip = clip.cuda(non_blocking=True)
            #images = rearrange(images, 'B C D H W -> B D C H W')
            recon, cluster_loss, space_loss = model(clip)
            #utils.save_tensor_video(recon, output_dir='test_video_show')
            #cluster_loss = torch.mean(cluster_loss, [2, 3, 4])
            loss = recon_loss(recon, clip)
            loss = rearrange(loss, 'B C D H W -> B D C H W')
            loss = torch.squeeze(loss, dim=0)
            loss_frame = torch.mean(loss, dim=3).mean(dim=2).mean(dim=1)
            loss_frame = loss_frame.tolist()
            psnr_frame = utils.psnr(loss_frame)
            ...
            label_clip = label_clip.tolist()[0][0:len(psnr_frame)]
